...
function picture_go(){
   ...
   //업로드 확인변수 초기화 (사진변경)
   form.find('[name="ckeckUpload"]').val(0);
   document.getElementById('inputFileName').value=picture.files[0].name;
   //jQuery : $('#inputFileName').val(picture.files[0].name);
...
function upload_go(){
	...
	if($('input[name="pictureFile"]').val()){
		alert("사진을 선택하세요.");
		$('input[name="pictureFile"]').click(); // 사진 클릭으로 바로 넘어감
		return;
	}
	...
	//.[0]을 붙여 jQuery객체를 자바스크립트 객체로 바꿔서 넣어줌
	var formData = new FormData($('form[role="imageForm"]')[0]);
	
	$.ajax({
	      url : "picture",//어디로 보낼건지
	      data : formData,
	      type : "post",
	      processData : false,
	      contentType : false,
